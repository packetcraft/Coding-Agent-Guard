...
On macOS, Cursor is typically installed as `/Applications/Cursor.app`. The current probe only checks `~/.cursor` (the home-dir config/data directory), which is created after Cursor has been launched at least once.

**Impact:** A fresh Cursor install that has never been opened will not be detected on macOS.  
**Workaround:** Launch Cursor once so it creates `~/.cursor`, then re-run the scan.  
**Future fix:** Add a probe for `/Applications/Cursor.app` existence on `darwin`.
